...
security_bp = Blueprint('Seguranca', __name__, url_prefix='/seguranca')
SISTEMA_ID = int(os.getenv("SISTEMA_ID", 3)) # Lembre-se que o ID do Integrador é 3 (ou o que você definiu)

@security_bp.route('/configuracoes', methods=['GET'])
@login_required
def Configuracoes():
    PodeVerPermissoes = True if DEBUG_PERMISSIONS else PermissaoService.VerificarPermissao(current_user, 'INTEGRADOR.SEGURANCA.VISUALIZAR')
    return render_template('Pages/Configs/Configuracoes.html', PodeVerPermissoes=PodeVerPermissoes)

@security_bp.route('/gerenciador', methods=['GET'])
@login_required
@RequerPermissao('INTEGRADOR.SEGURANCA.VISUALIZAR')
def VisualizarGerenciador():
    Sessao = GetSqlServerSession()
    try:
        # Puxando dados REAIS do seu banco de dados
        ...
        PodeEditar = True if DEBUG_PERMISSIONS else PermissaoService.VerificarPermissao(current_user, 'INTEGRADOR.SEGURANCA.EDITAR')
        PodeCriar  = True if DEBUG_PERMISSIONS else PermissaoService.VerificarPermissao(current_user, 'INTEGRADOR.SEGURANCA.CRIAR')
...
def BuscarAcessosUsuario():
    ...
    try:
        ...
        return jsonify({
            ...
            'usuario_permitidos': [o.Id_Permissao for o in Overrides if o.Conceder], 
            'usuario_bloqueados': [o.Id_Permissao for o in Overrides if not o.Conceder]
        })
    finally: 
        Sessao.close()
...
@login_required
@RequerPermissao('INTEGRADOR.SEGURANCA.EDITAR')
def SalvarVinculo():
    Dados = request.get_json()
    IdAlvo = int(Dados.get('IdAlvo'))
    IdPermissao = int(Dados.get('IdPermissao'))
    Tipo = Dados.get('Tipo')
    Acao = Dados.get('Acao')
...
@login_required
@RequerPermissao('INTEGRADOR.SEGURANCA.CRIAR')
